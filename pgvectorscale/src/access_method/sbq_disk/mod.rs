...
use storage::SbqDiskSpeedupStorage;
...
pub type SbqDiskVectorElement = u64;

#[derive(Archive, Deserialize, Serialize, Readable, Writeable)]
#[archive(check_bytes)]
#[repr(C)]
pub struct SbqDiskMeansV1 {
    count: u64,
    means: Vec<f32>,
    m2: Vec<f32>,
}
...
impl SbqDiskMeansV1 {
    pub unsafe fn load<S: StatsNodeRead>(
        ...
        let bq = SbqDiskMeansV1::read(index, qip, stats);
    ...
    pub unsafe fn store<S: StatsNodeWrite>(
        ...
        let mut tape = Tape::new(index, PageType::SbqDiskMeans);
        let node = SbqDiskMeansV1 {
...
impl SbqDiskMeans {
    pub unsafe fn load<S: StatsNodeRead>(
        ...
        match page_type {
            PageType::SbqDiskMeansV1 => SbqDiskMeansV1::load(index, quantizer, qip, stats),
            PageType::SbqDiskMeans => {
                let mut tape_reader = ChainItemReader::new(index, PageType::SbqDiskMeans, stats);
                let mut buf: Vec<u8> = Vec::new();
                for item in tape_reader.read(qip) {
                    buf.extend_from_slice(item.get_data_slice());
                }

                let means = rkyv::from_bytes::<SbqDiskMeans>(buf.as_slice()).unwrap();
                quantizer.load(means.count, means.means, means.m2);
                quantizer
            }
            _ => {
                pgrx::error!("Invalid page type {} for SbqDiskMeans", page_type as u8);
            }
        }
    }
    ...
    pub unsafe fn store<S: StatsNodeWrite>(
        ...
        let mut tape = ChainTapeWriter::new(index, PageType::SbqDiskMeans, stats);
        let buf = rkyv::to_bytes::<_, 1024>(&bq).unwrap();
        tape.write(&buf)
    }
}

pub struct SbqDiskSearchDistanceMeasure {
    vec: Vec<SbqDiskVectorElement>,
    query: LabeledVector,
}

impl SbqDiskSearchDistanceMeasure {
...
pub struct SbqDiskNodeDistanceMeasure<'a> {
    vec: Vec<SbqDiskVectorElement>,
    storage: &'a SbqDiskSpeedupStorage<'a>,
}

impl<'a> SbqDiskNodeDistanceMeasure<'a> {
...
impl NodeDistanceMeasure for SbqDiskNodeDistanceMeasure<'_> {

...
use super::{
    ...
    node::{ArchivedSbqDiskNode, SbqDiskNode},
    SbqDiskMeans, SbqDiskNodeDistanceMeasure, SbqDiskSearchDistanceMeasure,
};

use crate::access_method::sbq::quantize::SbqQuantizer;

pub struct SbqDiskSpeedupStorageLsnPrivateData;

pub struct SbqDiskSpeedupStorage<'a> {
...
impl<'a> SbqDiskSpeedupStorage<'a> {
    ...
    pub unsafe fn new_for_build<S: StatsNodeRead>(
        ...
        let quantizer = SbqDiskMeans::load(index, meta_page, stats);
        Self {
            index,
    ...
    fn load_quantizer<S: StatsNodeRead>(
        ...
        unsafe { SbqDiskMeans::load(index_relation, meta_page, stats) }
    }
    ...
    fn visit_lsn_internal(
        ...
        lsr: &mut ListSearchResult<
            <SbqDiskSpeedupStorage<'a> as Storage>::QueryDistanceMeasure,
            <SbqDiskSpeedupStorage<'a> as Storage>::LSNPrivateData,
        >,
        ...
        match gns {
            GraphNeighborStore::Disk => {
                ...
                let neighbors =
                    node_visiting.get_index_pointer_to_neighbors(self.index, &mut lsr.stats);
                ...
                for &neighbor_index_pointer in neighbors.iter() {
                    ...
                    let lsn = ListSearchNeighbor::new(
                        ...
                        SbqDiskSpeedupStorageLsnPrivateData,
...
impl Storage for SbqDiskSpeedupStorage<'_> {
    type QueryDistanceMeasure = SbqDiskSearchDistanceMeasure;
    type NodeDistanceMeasure<'a>
        = SbqDiskNodeDistanceMeasure<'a>
    where
        Self: 'a;
    type ArchivedType<'b>
        = ArchivedSbqDiskNode<'b>
    where
        Self: 'b;
    type LSNPrivateData = SbqDiskSpeedupStorageLsnPrivateData; //no data stored

    fn page_type() -> PageType {
        PageType::SbqDiskNode
    }
    ...
    fn finalize_node_at_end_of_build<S: StatsNodeRead + StatsNodeModify>(
        ...
        if you try to fetch a quantized vector while holding the SbqDiskNode::modify lock */
        let iter = neighbors
            .iter()
        ...
        let mut node =
            unsafe { SbqDiskNode::modify(self.index, index_pointer, self.has_labels, stats) };
        let mut archived = node.get_archived_node();
        archived.set_neighbors(neighbors, self.num_neighbors, self.index, stats);
        node.commit();
    }
    ...
    unsafe fn get_node_distance_measure<'b, S: StatsNodeRead + StatsNodeWrite + StatsNodeModify>(
        ...
        index_pointer: IndexPointer,
        stats: &mut S,
    ) -> SbqDiskNodeDistanceMeasure<'b> {
        SbqDiskNodeDistanceMeasure::with_index_pointer(self, index_pointer, stats)
    }

    fn get_query_distance_measure(&self, query: LabeledVector) -> SbqDiskSearchDistanceMeasure {
        SbqDiskSearchDistanceMeasure::new(&self.quantizer, query)
    }
    ...
    fn get_neighbors_with_distances_from_disk<S: StatsNodeRead + StatsDistanceComparison>(
        ...
        let rn = unsafe { SbqDiskNode::read(self.index, neighbors_of, self.has_labels, stats) };
        let archived = rn.get_archived_node();
        let q = archived.get_bq_vector();

        let neighbors: Vec<_> = rn
            .get_archived_node()
            .iter_neighbors(self.index, stats)
            .collect();
        ...
        neighbors
            ...
            .map(|n| {
                ...
                let rn1 = unsafe { SbqDiskNode::read(self.index, n, self.has_labels, stats) };
    ...
    fn create_lsn_for_start_node(
        ...
        let rn = unsafe {
            SbqDiskNode::read(self.index, index_pointer, self.has_labels, &mut lsr.stats)
        };
    ...
    fn return_lsn(
        ...
        let rn =
            unsafe { SbqDiskNode::read(self.index, lsn_index_pointer, self.has_labels, stats) };
        let node = rn.get_archived_node();
    ...
    fn get_labels<S: StatsNodeRead>(
        ...
        let rn = unsafe { SbqDiskNode::read(self.index, index_pointer, self.has_labels, stats) };
        let node = rn.get_archived_node();
        node.get_labels().map(Into::into)
    }
}
